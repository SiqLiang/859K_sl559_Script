...
print (os.path.exists("C:\\859K_sl559\\Doc\\DEMFC30_DeFacto_RasterArea.txt"))
# Create a new file in the current working directory, write some text, and remember to close it
BL_Map_fileObj = open("DEMFC30_DeFacto_RasterArea.txt",'w')
BL_Map_fileObj.truncate(0) # clear eveything already in the txt file
BL_Map_fileObj.write('DEMFC30_DeFacto_Species, '+'Area_sqkm2'+"\n")
...
fileList = arcpy.ListRasters('DEMFC30_DeFacto_*', 'All')
print(len(fileList))
for fileName in fileList:
    print(fileName)
    scientific_name = fileName[16:fileName.rfind('.')]
